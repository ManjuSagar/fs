class ClinicalCorrectionWorkflow < OasisDocumentWorkflow
  aasm do
    ...
    event :approve, :before => [:set_agency_approved_user, :add_approved_document_event_to_notes] do
      transitions :from => :pending_qa, :to => :exported, :guard => lambda {|d| d.valid_document? and d.current_user_is_office_staff? }
    end
    ...
    event :request_correction, :before => :add_corrrection_request_document_event_to_notes do
      transitions :from => :pending_qa, :to => :draft, :on_transition => Proc.new {|d, *args| d.set_as_fs_not_signed }, :guard => lambda {|d| d.valid_document?  and d.current_user_is_office_staff? and d.field_signature_required? and d.system_driven_event? }
