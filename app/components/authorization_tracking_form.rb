class AuthorizationTrackingForm <  Mahaswami::FormPanel
    ...
    s.merge(
        ...
        items: [
            ...
            },
            {
                ...
                margin: "5%",
                items:[
                    { name: :treatment_episode__to_s, flex: 0.5, field_label: "Episode", allow_blank: false, width: '50%', label_align: 'right',
                        label_width: '62%', item_id: :episode_id},
  ...
  def treatment_episode__to_s_combobox_options(params)
    list = []
    treatment = Patient.org_scope.where(id: component_session[:patient_id]).first.treatments.last
    list = treatment.treatment_episodes.collect {|e| [e.id, e.certification_period]} unless treatment.nil?
    {data: list.reverse}
  end
