...
office_staffs =  CSV.readlines("#{Rails::root}/static_data/gentle_care_agency_setup/office_staff.csv", :headers=>true, skip_blanks: true)
visit_types =  CSV.readlines("#{Rails::root}/static_data/gentle_care_agency_setup/visit_types.csv", :headers=>true, skip_blanks: true)
order_types =  CSV.readlines("#{Rails::root}/static_data/gentle_care_agency_setup/order_types.csv", :headers => true, skip_blanks: true)
attachment_types =  CSV.readlines("#{Rails::root}/static_data/gentle_care_agency_setup/attachment_types.csv", :headers => true, skip_blanks: true)
insurance_companies = CSV.readlines("#{Rails::root}/static_data/gentle_care_agency_setup/insurance_companies.csv", :headers => true, skip_blanks: true)
...
free_form_templates = CSV.readlines("#{Rails::root}/static_data/gentle_care_agency_setup/templates.csv", :headers=>true, skip_blanks: true)
...
ActiveRecord::Base.transaction do
  ...
  email = is_production ? "hfofang@yahoo.com" : get_email("hfofang")
  agency_num = "106"
  health_agency = HealthAgency.new
  health_agency.org_name = "GentleCare Home Health1, LLC"
  health_agency.org_type = "HealthAgency"
  health_agency.org_package = 'P'
  health_agency.week_start_day = 'SUN'
  health_agency.suite_number = ""
  health_agency.street_address = "4525 cleveland DR"
  health_agency.city = "Plano"
  health_agency.state = 'TX'
  health_agency.zip_code = "75093"
  health_agency.preferred_alert_mode = 'E'
  health_agency.email = email
  health_agency.phone_number = "(214) 905-1414"
  health_agency.fax_number = "(214) 905-3441"
  health_agency.fed_tax_number = "113720147"
  #health_agency.npi_number = "123456789"
  health_agency.primary_contact_first_name = "Henry"
  health_agency.primary_contact_last_name = "Fofang"
  health_agency.primary_contact_email = email
  health_agency.primary_contact_phone_number = "(214) 905-1414"
  health_agency.primary_contact_password = "password"
  health_agency.primary_contact_password_confirmation = "password"
  health_agency.save!
  debug_log health_agency.valid?
  debug_log health_agency.errors
  ...
  health_agency_detail.cms_cert_number = "679542"
  health_agency_detail.npi_number = "1285729376"
  health_agency_detail.health_agency_id = health_agency.id
  health_agency_detail.submitter_id = "sw23105"
  ...
  visit_types.each do |vt_row|
    ...
    visit_type.discipline = Discipline.find_by_discipline_code(vt_row[2])
    visit_type.sort_order = vt_row[4]
    license_type_codes = vt_row[3].split(', ')
    license_type_codes.each do |license_type_code|
      debug_log "license type code.........#{license_type_code}"
...
=end
    ...
    transitions = {"PE-AC" => vt_row[7], "PE-DC" => vt_row[8], "AC-DC" => vt_row[9], "HD-AC" => vt_row[10],
                   "HD-DC" => vt_row[11], "AS-AS" => vt_row[6]}
