...
ActiveRecord::Base.transaction do
  ...
  health_agency.org_name = "Island Agency"
  ...
  staffing_companies.each_with_index do |sc_row, index|
    ...
    visit_types.each do |visit_type|
      ...
      visit_type_rate.chargeable_rate = 100
  ...
  insurance_companies.each do |row|
    ...
    ins = InsuranceCompany.new(company_name: row[0].strip, company_code: row[1].strip, claim_street_address: row[2],
                               claim_suite_number: row[3], claim_zip_code: row[4], claim_city: row[5],
  ...
  visit_type_rates.update_all("rate = 100")
